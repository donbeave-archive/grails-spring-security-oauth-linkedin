...
'''
    ...
    private void loadConfig(ConfigObject config) {
        ...
        def newConfig = new ConfigSlurper(Environment.current.name).parse(
                classLoader.loadClass('DefaultLinkedinOauthConfig')
        )

        newConfig.oauth.providers.linkedin.merge(config.oauth.providers.linkedin)

        // Now add DefaultLinkedinOauthConfig into the main config
